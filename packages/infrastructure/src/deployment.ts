            ...
            template: {
                ...
                spec: {
                    ...
                    containers: [{
                        ...
                        name: appName,
                        image: `thetechcompany/hivechat-backend:${imageTag}`,
                        ports: [{ name: "http", containerPort: 9015 }],
                        volumeMounts: [
                        ],
                        env: [
                            ...
                            { name: "DATABASE_URL", value: all([dbUrl, dbPass]).apply(([url, pass]) => `postgresql://postgres:${pass}@${url}.db-${suffix}.svc.cluster.local:5432/hivechat`) },
